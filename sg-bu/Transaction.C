    ...
    transactionCount_(0)
{
  ...
  LOG_ERROR("Transaction::Impl() ptr: " << connection_.get() << " " << transactionCount_); 
}
...
Transaction::Impl::~Impl()
{
  ...
  LOG_ERROR("Transaction::close() ptr: " << connection_.get()<< " " << transactionCount_);
}
...
void Transaction::Impl::open()
{
  if (!open_) {
    ...
    LOG_ERROR("Transaction::open() ptr: " << connection_.get()<< " " << transactionCount_);
  }
}
...
void Transaction::Impl::rollback()
{
  ...
  try {
    if (open_)
    	LOG_ERROR("Transaction::rollback() ptr: " << connection_.get() << " " << transactionCount_);
      connection_->rollbackTransaction();
  } catch (const std::exception& e) {
    LOG_ERROR("Transaction::rollback(): " << e.what() << " - " << connection_.get() << " " << transactionCount_);
  }
