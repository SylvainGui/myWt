...
namespace http
{
  namespace server
  {
    ...
    ProxyReply::ProxyReply(uWS::HttpRequest* request,
    ...
    void ProxyReply::writeDone(bool success)
    {
      ...
      if (more_ && socket_)
      {
        ...
        asio::async_read(*socket_, responseBuf_,
                         ...
                         connection()->strand().wrap(std::bind(&ProxyReply::handleResponseRead,
                                 ...
                                 Request::State state)
    {
      ...
      if (sessionProcess_)
      {
        if (socket_)
        {
          ...
          //                   connection()->strand().wrap(std::bind(&ProxyReply::handleDataWritten,
          //                                                         std::static_pointer_cast<ProxyReply>(shared_from_this()),
          //                                                         std::placeholders::_1,
          //                                                         std::placeholders::_2)));
        }
      ...
      else
      {
        ...
        if (sessionId.empty() || !sessionProcess_)
        {
          ...
          auto wtt = request_->getQuery("wtt");

          if (!sessionId.empty() && (wtt.empty() || wtt != "widgetset"))
          {
            /*
            ...
            */
            ...
            auto request = request_->getQuery("request");

            if (!request.empty())
            {
              if (request == "resource" || request == "style")
              {
          ...
          if (sessionManager_.tryToIncrementSessionCount())
          {
            ...
            sessionProcess_->asyncExec(
                configuration(),
                connection()->strand().wrap(std::bind(&ProxyReply::connectToChild,
                                                      std::static_pointer_cast<ProxyReply>(shared_from_this()),
                                                      std::placeholders::_1)));
    ...
    void ProxyReply::connectToChild(bool success)
    {
      if (success)
      {
        ...
        socket_->async_connect(sessionProcess_->endpoint(),
                               connection()->strand().wrap(std::bind(&ProxyReply::handleChildConnected,
                                                                     std::static_pointer_cast<ProxyReply>(shared_from_this()),
                                                                     std::placeholders::_1)));
      }
      else
    ...
    {
      std::ostream os(&requestBuf_);
      os << request_->getMethod() << " " << request_->getUrl() << " HTTP/1.1\r\n";
      bool establishWebSockets = false;
      std::string forwardedFor;
      std::string forwardedProto = "http";
      ...
      {
        if ((*it).first == "Connection" || (*it).first == "Keep-Alive" ||
            (*it).first == "TE" || (*it).first == "Transfer-Encoding")
        {
          // Remove hop-by-hop header
        }
        else if ((*it).first == SSL_CLIENT_CERTIFICATES_HEADER)
        {
        ...
        else if ((*it).first == "X-SSL-Client-")
        {
          ...
          {
            LOG_SECURE("wthttp is not behind a trusted reverse proxy, dropping " << (*it).first << " header");
          }
        }
        else if ((*it).first == wtConfiguration.originalIPHeader().c_str())
        {
          if (trustedProxy)
          {
            forwardedFor = std::string((*it).second) + ", ";
          }
          else
          {
            LOG_SECURE("wthttp is not behind a trusted reverse proxy, dropping " << (*it).first << " header");
        ...
        else if ((*it).first == "X-Forwarded-Proto")
        {
          if (trustedProxy)
          {
            forwardedProto = (*it).second;
          }
          else
          {
            LOG_SECURE("wthttp is not behind a trusted reverse proxy, dropping " << (*it).first << " header");
        ...
        else if ((*it).first == "X-Forwarded-Port")
        {
          if (trustedProxy)
          {
            forwardedPort = (*it).second;
          }
          else
          {
            LOG_SECURE("wthttp is not behind a trusted reverse proxy, dropping " << (*it).first << " header");
        ...
        else if ((*it).first == "X-Forwarded-Host")
        {
          if (trustedProxy)
          {
            forwardedHost = (*it).second;
          }
          else
          {
            LOG_SECURE("wthttp is not behind a trusted reverse proxy, dropping " << (*it).first << " header");
      ...
      if (!ec)
      {
        ...
        else
        {
          asio::async_read_until(*socket_, responseBuf_, "\r\n",
                                 connection()->strand().wrap(std::bind(&ProxyReply::handleStatusRead,
                                                                       std::static_pointer_cast<ProxyReply>(shared_from_this()),
                                                                       std::placeholders::_1)));
        }
      }
    ...
    void ProxyReply::handleStatusRead(const Wt::AsioWrapper::error_code &ec)
    {
      if (!ec)
      {
        ...
        asio::async_read_until(*socket_, responseBuf_, "\r\n\r\n",
                               connection()->strand().wrap(std::bind(&ProxyReply::handleHeadersRead,
                                                                     std::static_pointer_cast<ProxyReply>(shared_from_this()),
                                                                     std::placeholders::_1)));
      }
      else
    ...
    void ProxyReply::handleResponseRead(const Wt::AsioWrapper::error_code &ec)
    {
      ...
      else if (ec == asio::error::eof || ec == asio::error::shut_down || ec == asio::error::operation_aborted || ec == asio::error::connection_reset)
      {
        closeClientSocket();

        more_ = false;

        if (request_->isAncient()) //request_.type != Request::TCP
        {
          send();
        }
      }
    ...
    std::string ProxyReply::getSessionId() const
    {
      ...
      {
        auto cookie = request_->getHeader("Cookie");
        if (!cookie.empty())
        {
          //std::string cookie = cookieHeader->value.str();
          sessionId = Wt::WebController::sessionFromCookie(cookie, request_->getUrl(),
    ...
    void ProxyReply::error(status_type status)
    {
      ...
      if (request_->isAncient()) //request_.type == Request::HTTP
      {
        setStatus(status);
        setCloseConnection();
        more_ = false;
    ...
    {
      bool jsRequest = request_->getMethod() == "POST" && queryParams_.size() == 1;
      ...
      if (jsRequest)
      {
        ...
        auto horigin = request_->getHeader("Origin");
        std::string origin = horigin.empty() ? "*" : std::string(horigin);
        ...
        out_ << "if (window.Wt) window.Wt._p_.quit(null); window.location.reload(true);";
        more_ = false;
        send();
        closeClientSocket();
