...
Reply::Reply(uWS::HttpRequest* request, const Configuration& config)
  : request_(request),
...
bool Reply::nextBuffers(std::vector<asio::const_buffer>& result)
{
  ...
  else
  {
    ...
    {
      transmitting_ = true;
      bool http10 = false;// (request_.http_version_major == 1) && (request_.http_version_minor == 0);

      closeConnection_ = closeConnection_ || request_->getHeader("Connection") == "close"; //request_.closeConnection()
       ...
       * Connection
       */
      if (closeConnection_ && request_->isAncient()) //request_.type == Request::HTTP
      {
...
void Reply::receive()
{
  connection_->strand().post(std::bind(&Connection::readMore, connection_,
...
void Reply::send()
{
  ...
  buf_.asioBuffers(result);
    response_->write(buf_.c_str());

    // We post this since we want to avoid growing the stack indefinitely
    connection_->server()->service().post(connection_->strand().wrap(std::bind(&Connection::startWriteResponse, connection_,
                                                                               shared_from_this())));
  }
}
