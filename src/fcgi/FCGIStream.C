...
namespace
{
  ...
  public:
    ...
    virtual void addHeader(const std::string &name, std::string_view value) override
    {
      if (!headersCommitted_)
        *out_ << name << ": " << value << "\r\n";
        ...
        LOG_WARN("addHeader(): " << name << ": " << value.data()
    ...
    std::vector<Wt::Http::Message::Header> headers() const override
    {
      ...
      std::string_view contentLength = envValue("CONTENT_LENGTH");
      if (!contentLength.empty())
      {
        headerVector.push_back(Wt::Http::Message::Header("CONTENT_LENGTH", contentLength.data()));
      }
      std::string_view contentType = envValue("CONTENT_TYPE");
      if (!contentType.empty())
      {
        headerVector.push_back(Wt::Http::Message::Header("CONTENT_TYPE", contentType.data()));
      }
      return headerVector;
    }
    ...
    virtual std::string_view scriptName() const override
    {
      ...
      {
        if (entryPoint_)
          scriptName_ = std::string(envValue("SCRIPT_NAME")) + std::string(entryPoint_->path());
        else
          scriptName_ = std::string(envValue("SCRIPT_NAME"));
      }
    ...
    virtual std::string_view serverName() const override
    {
      if (serverName_.empty())
        serverName_ = std::string(envValue("SERVER_NAME"));
    ...
    virtual std::string_view queryString() const override
    {
      if (queryString_.empty())
        queryString_ = std::string(envValue("QUERY_STRING"));
    ...
    virtual std::string_view serverPort() const override
    {
      if (serverPort_.empty())
        serverPort_ = std::string(envValue("SERVER_PORT"));
    ...
    virtual std::string_view pathInfo() const override
    {
      if (pathInfo_.empty())
      {
        pathInfo_ = std::string(envValue("PATH_INFO"));
        if (entryPoint_)
        {
    ...
    virtual std::string_view remoteAddr() const override
    {
      if (remoteAddr_.empty())
        remoteAddr_ = std::string(envValue("REMOTE_ADDR"));
    ...
    {
      const char *https = envValue("HTTPS").data();
    ...
    {
#ifdef WT_WITH_SSL
      std::string clientCert = std::string(envValue("SSL_CLIENT_CERT"));
      if (!clientCert.empty())
      {
        ...
        if (x509)
        {
          ...
          std::string verify = std::string(envValue("SSL_CLIENT_VERIFY"));
