...
namespace Wt
{
  ...
  void WIOService::start()
  {
    ...
    if (!impl_->work_)
    {
      ...
      {
        impl_->threads_.push_back(std::make_unique<std::thread>(std::bind(&WIOService::run, this)));
      }
  ...
  void WIOService::schedule(std::chrono::steady_clock::duration millis, const std::function<void()> &function)
  {
    ...
    else
    {
      ...
      timer->async_wait(std::bind(&WIOService::handleTimeout, this, timer, function,
                                  std::placeholders::_1));
    }
  }
