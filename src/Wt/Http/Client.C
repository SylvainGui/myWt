...
public:
  ...
  Impl(asio::io_service& ioService, WServer *server,
    ...
    : ioService_(ioService),
      #if MODERN
      strand_(asio::make_strand(ioService)),
      #else
      strand_(ioService),
      #endif
	       ...
	       const Message& message)
  {
    ...
    resolver_.async_resolve
      (query,
       bind_executor(strand_, [this] (AsioWrapper::error_code err, tcp::resolver::iterator endpoint_iterator) 
       { 
  ...
  void asyncStop(std::shared_ptr<Impl> *impl)
  {
    ...
    ioService_.post
      (bind_executor(strand_, [this, impl] { stop(impl); } ));
  }
...
private:
  ...
  void startTimer()
  {
    ...
    //   (strand_.wrap(std::bind(&Impl::timeout, shared_from_this(),
      ...
      timer_.async_wait
      (bind_executor(strand_, [this] (AsioWrapper::error_code e) { timeout(e); } ));
  }
		     ...
		     tcp::resolver::iterator endpoint_iterator)
  {
    ...
    if (!err && !aborted_) {
      ...
      ++endpoint_iterator;
      asyncConnect(endpoint,
                   bind_executor(strand_, [this, endpoint_iterator] (AsioWrapper::error_code err)
                   {
                     handleConnect(err, endpoint_iterator);
                   }
                   ));
    } else {
      if (aborted_)
		     ...
		     tcp::resolver::iterator endpoint_iterator)
  {
    ...
    if (!err && !aborted_) {
      ...
      asyncHandshake
      (bind_executor(strand_, [this] (AsioWrapper::error_code err){handleHandshake(err);} ));
  ...
  void handleHandshake(const AsioWrapper::error_code& err)
  {
    ...
    if (!err && !aborted_) {
      ...
      asyncWriteRequest(bind_executor(strand_, [this] (AsioWrapper::error_code err, size_t size) 
                        {
                          handleWriteRequest(err, size);
                        }));
    } else {
      if (aborted_)
        err_ = asio::error::operation_aborted;
  ...
  //     asyncReadUntil
	...
	// 	      std::placeholders::_1,
	// 	      std::placeholders::_2)));
      asyncReadUntil("\r\n", bind_executor(strand_, [this] (AsioWrapper::error_code err, size_t size)
                    {
                      handleReadStatusLine(err, size);
                    }));
			 ...
			 const std::size_t& s)
  {
	...
	// 	       std::placeholders::_1,
	// 	       std::placeholders::_2)));
        asyncRead(bind_executor(strand_, [this] (AsioWrapper::error_code err, size_t size)
                  {
                    handleReadContent(err, size);
                  }));
      } else {
	complete();
      }
