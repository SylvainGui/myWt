  ...
  namespace Http
  {
    ...
    WT_BOSTREAM &Response::out()
    {
      if (!headersCommitted_)
      {
        if (response_ &&
            !continuation_ &&
            (resource_->dispositionType() != ContentDisposition::None || !resource_->suggestedFileName().empty()))
        {
          WStringStream cdp;
          ...
          if (!fileName.empty())
          {
            ...
            bool isIE = iequals(ua, "MSIE");
            bool isChrome = iequals(ua.data(), "Chrome");
    ...
    Response& Response::operator<< (std::string_view toclient)
    {
      *response_ << toclient;
      return *this;
    }
