...
namespace Wt
{
  namespace Http
  {
    ...
    {
      return request_ ? std::string(request_->requestMethod()) : str("GET");
    }
    ...
    {
      return request_ ? std::string(request_->headerValue(field.c_str())) : std::string();
    }

    std::string Request::getCgiValue(const std::string &varName) const
    {
      return request_ ? std::string(request_->envValue(varName.c_str())) : std::string();
    }
    ...
    {
      return request_ ? std::string(request_->contentType()) : std::string();
    }
    ...
    {
      return request_ ? std::string(request_->userAgent()) : std::string();
    }
      ...
      for (std::size_t pos = 0; pos < s.length();)
      {
        ...
        if (next == std::string::npos || s[next] == '&')
        {
          if (next == std::string::npos)
            next = s.length();
          std::string key = std::string(s.substr(pos, next - pos));
        ...
        else
        {
          ...
          std::string value = std::string(s.substr(next + 1, amp - (next + 1)));
