...
void WEnvironment::setInternalPath(std::string_view path)
{
  if (path.empty())
    internalPath_ = std::string(path);
  else
...
void WEnvironment::updateHostName(const WebRequest &request)
{
  ...
  host_ = std::string(request.headerValue("host"));

  if (conf.behindReverseProxy() ||
      conf.isTrustedProxy(request.remoteAddr()))
  {
    auto forwardedHost = (request.headerValue("x-forwarded-host"));
...
void WEnvironment::updateUrlScheme(const WebRequest &request)
{
  ...
  if (conf.behindReverseProxy() ||
      conf.isTrustedProxy(request.remoteAddr()))
  {
    std::string forwardedProto = std::string(request.headerValue("x-forwarded-proto"));
...
void WEnvironment::init(const WebRequest& request)
{
  ...
  host_            = std::string(request.headerValue("host"));
  referer_         = std::string(request.headerValue("referer"));
  accept_          = std::string(request.headerValue("accept"));
  serverSignature_ = std::string(request.envValue("SERVER_SIGNATURE"));
  serverSoftware_  = std::string(request.envValue("SERVER_SOFTWARE"));
  serverAdmin_     = std::string(request.envValue("SERVER_ADMIN"));
  pathInfo_        = request.pathInfo();

#ifndef WT_TARGET_JAVA
  if(!request.headerValue("redirect-secret").empty())
	session_->controller()->redirectSecret_ = std::string(request.headerValue("redirect-secret"));

  sslInfo_ = request.sslInfo(conf);
#endif

  setUserAgent(std::string(request.headerValue("user-agent")));
  ...
  if (host_.empty()) {
    ...
    if (!request.serverPort().empty())
      host_ += ":" + std::string(request.serverPort());
  }
                                ...
                                std::unordered_map<std::string, std::string> &result)
{
  ...
  for (unsigned int i = 0; i < list.size(); ++i)
  {
    ...
    std::string cookieValue =
        (e != std::string::npos && list[i].size() > e + 1) ? list[i].substr(e + 1) : "";
