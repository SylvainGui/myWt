...
namespace Wt
{
                                               ...
                                               const int sessionIdLength)
  {
    ...
    const char *start = cookies.begin();
    const char *const end = cookies.end();
                                          ...
                                          std::uintmax_t total)
  {
    ...
    if (uploadProgressUrls_.find(request->queryString().data()) != uploadProgressUrls_.end())
    {
#ifdef WT_THREADED
      lock.unlock();
#endif // WT_THREADED
                                             ...
                                             std::uintmax_t total)
  {
    ...
    if (!requestE && !request->pathInfo().empty())
      resource = app->decodeExposedResource("/path/" + std::string(request->pathInfo()));
  ...
  void WebController::handleRequest(WebRequest *request)
  {
    ...
    {
      request
          ->entryPoint_->resource()
          ->handle(request, (WebResponse *)request);
      return;
    }
    ...
    if (requestE && *requestE == "redirect")
    {
      ...
      else
      {
        ...
        *request
            << "<title>Error occurred.</title>"
            << "<h2>Error occurred.</h2><p>Invalid redirect.</p>\n";
      }
   ...
   */
    ...
    if (conf_.sessionTracking() == Configuration::Combined)
      multiSessionCookie = sessionFromCookie(request->headerValue("cookie"),
                                             "ms" + std::string(request->scriptName()),
    ...
    std::shared_ptr<WebSession> session;
    {
           ...
           (multiSessionCookie.empty() || multiSessionCookie != i->second->multiSessionId())))
      {
        try
        {
          ...
          {
            if (request->headerValue("cookie").empty())
            {
          ...
          if (sessionTracking == Configuration::CookiesURL)
            request->addHeader("Set-Cookie",
                               appSessionCookie(request->scriptName()) + "=" + sessionId +
                                   "; Version=1;" + " Path=" + session->env().deploymentPath() +
  ...
  std::unique_ptr<WApplication> WebController ::doCreateApplication(WebSession *session)
  {
  ...
  {
    auto scriptName = request->scriptName();
  ...
  bool WebController::limitPlainHtmlSessions()
  {
    if (conf_.maxPlainSessionsRatio() > 0)
    {
      ...
      if (plainHtmlSessions_ + ajaxSessions_ > 20)
        return plainHtmlSessions_ > conf_.maxPlainSessionsRatio() * (ajaxSessions_ + plainHtmlSessions_);
      else
        return false;
    }
    else
