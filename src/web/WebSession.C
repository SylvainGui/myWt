...
namespace Wt
{
  ...
  void WebSession::init(const WebRequest &request)
  {
...
#else
    std::string *absoluteBaseUrl = app_->readConfigurationProperty("baseURL", absoluteBaseUrl_);
    ...
    if (useAbsoluteUrls)
    {
      ...
      if (slashpos != std::string::npos && slashpos != absoluteBaseUrl_.length() - 1)
  ...
  std::string WebSession::appendSessionQuery(const std::string &url) const
  {
     ...
     */
      ...
      return result.substr(0, questionPos) + ";jsessionid=" + sessionId() + result.substr(questionPos);
    }
#endif // WT_TARGET_JAVA
  }
  ...
  std::string WebSession::getCgiValue(const std::string &varName) const
  {
    ...
    if (request)
      return std::string(request->envValue(varName.c_str()));
  ...
  std::string WebSession::getCgiHeader(const std::string &headerName) const
  {
    WebRequest *request = WebSession::Handler::instance()->request();
    if (request)
      return request->headerValue(headerName.c_str()).data();
    else
      return std::string();
  }
  ...
  void WebSession ::Handler::attachThreadToSession(const std::shared_ptr<WebSession> &session)
  {
...
#ifndef WT_TARGET_JAVA
    if (webSocket_)
      webSocket_->readWebSocketMessage(std::bind(&WebSession::handleWebSocketMessage, shared_from_this(),
                                                 std::placeholders::_1));
  ...
  void WebSession::handleRequest(Handler &handler)
  {
    ...
    if (request.isWebSocketRequest())
    {
      ...
      if (!origin.empty() && (trustedOrigin == origin || 
      ...
      else
      {
        ...
        if (origin.empty())
        {
          LOG_ERROR("WebSocket request refused: Origin '" /*<< std::string(origin)*/ << "' not allowed (trusted origin is '" << trustedOrigin << "')");
        }
        else
        {
     ...
     */
          ...
          conf.isAllowedOrigin(origin))
      {
        ...
        if (iequals(request.requestMethod(), "OPTIONS"))
        {
          ...
          if (!requestHeaders.empty())
            response->addHeader("Access-Control-Allow-Headers", requestHeaders.data());
          handler.flushResponse();
    ...
    if (requestE && *requestE == "ws" && !request.isWebSocketRequest())
    {
      ...
      LOG_INFO("Connection: " << std::string(request.headerValue("connection")));
      LOG_INFO("Upgrade: " << std::string(request.headerValue("upgrade")));
      LOG_INFO("Sec-WebSocket-Version: "
               << std::string(request.headerValue("sec-websocket-version")));
   ...
   */
    ...
    if (!(requestForResource || iequals(request.requestMethod(), "post") || iequals(request.requestMethod(), "get")))
    {
   ...
   */
    if (env_->ajax() && iequals(request.requestMethod(), "get") && !requestForResource && !conf.reloadIsNewSession() //todo
        && wtdE && *wtdE == sessionId_)
    {
   ...
   */
    ...
    {
      LOG_DEBUG("CSRF: " << (wtdE ? *wtdE : "no wtd") << " != " << sessionId_ << ", requestE: " << (requestE ? *requestE : "none"));
        ...
        case State::JustCreated:
        {
          ...
          case EntryPointType::WidgetSet:
            if (requestForResource)
            {
              ...
              {
                handler.response()->setContentType("text/html");
                *handler.response() << "<html><head><title>bhm</title></head>"
                                             "<body> </body></html>";
              }
              else
              {
          ...
          if (!app_)
          {
            ...
            else
            {
              ...
              else
              {
                ...
                {
                  handler.response()->setContentType("text/html");
                  *handler.response() << "<html><body><h1>Refusing to respond.</h1></body></html>";
                }
  ...
  void WebSession::handleWebSocketRequest(Handler &handler)
  {
    ...
    */
    ...
    //std::cout << "handleWebSocketRequest lock->canWriteWebSocket_ false" << std::endl;
    canWriteWebSocket_ = false;
    webSocketConnected_ = false;

    webSocket_->flush(WebRequest::ResponseState::ResponseFlush,
                                    ...
                                    WebWriteEvent event)
  {
    ...
    if (lock)
    {
      ...
      case WebWriteEvent::Completed:
        ...
        lock->webSocket_->flush(WebRequest::ResponseState::ResponseFlush,
                                std::bind(&WebSession::webSocketReady,
                                          std::weak_ptr<WebSession>(lock),
                                          std::placeholders::_1));

        lock->webSocket_->readWebSocketMessage(std::bind(&WebSession::handleWebSocketMessage,
                                                         std::weak_ptr<WebSession>(lock),
                                                         std::placeholders::_1));
                                          ...
                                          WebReadEvent event)
  {
    ...
    if (lock)
    {
      ...
      case WebReadEvent::Ping:
      {
        ...
        if (lock->canWriteWebSocket_)
        {
          //std::cout << " ping2 lock->canWriteWebSocket_ false" << std::endl;
        ...
        lock->webSocket_->readWebSocketMessage(std::bind(&WebSession::handleWebSocketMessage, session,
                                                         std::placeholders::_1));
      ...
      case WebReadEvent::Message:
      {
        ...
        bool closing = message->contentLength() == 0;
//lock->canWriteWebSocket_ = true; //need to manage the callback (protect )
        if (!closing)
        {
        ...
        if (!closing)
        {
          ...
          if (signalE && *signalE == "ping")
          {
            ...
            if (lock->canWriteWebSocket_)
            {
              //std::cout << " ping lock->canWriteWebSocket_ false" << std::endl;
              lock->canWriteWebSocket_ = false;
              lock->webSocket_->out() << "{}";
              lock->webSocket_->flush(WebRequest::ResponseState::ResponseFlush,
                                      std::bind(&WebSession::webSocketReady, session,
  ...
  std::string WebSession::ajaxCanonicalUrl(const WebResponse &request) const
  {
    ...
    if (!pagePathInfo_.empty() || (hashE && hashE->length() > 1))
    {
      ...
      for (Http::ParameterMap::const_iterator i = request.getParameterMap().begin();
           i != request.getParameterMap().end(); ++i)
      {
        if (i->first != "_")
        {
          url += (firstParameter ? '?' : '&') + Utils::urlEncode(i->first) + '=' + Utils::urlEncode(i->second[0]);
  ...
  void WebSession::pushUpdates()
  {
    ...
    if (asyncResponse_)
    {
      ...
      updatesPending_ = false;
      asyncResponse_->flush(); //delete if uwsocket
      asyncResponse_ = nullptr;
    }
    else if (webSocket_ && webSocketConnected_)
    {
        //std::cout << "webSocketMessagePending() ?" << std::endl;
      if (webSocket_->webSocketMessagePending())
      {
        LOG_DEBUG("pushUpdates(): web socket message pending");
        std::cout << "pushUpdates(): web socket message pending" << std::endl;
        return;
      }
      if (canWriteWebSocket_)
      {
...
//std::cout << " push update lock->canWriteWebSocket_ false" << std::endl;
        updatesPending_ = false;
        canWriteWebSocket_ = false;
        webSocket_->flush(WebRequest::ResponseState::ResponseFlush,
                          std::bind(&WebSession::webSocketReady,
                                  ...
                                  WebWriteEvent event)
  {
    ...
    if (lock)
    {
      ...
      case WebWriteEvent::Completed:
        if (lock->webSocket_)
        {
          //std::cout << "-----lock->canWriteWebSocket_ true-----" << std::endl;
      ...
      case WebWriteEvent::Error:
        if (lock->webSocket_)
        {
          //std::cout << "error ws ready lock->canWriteWebSocket_ false" << std::endl;
          lock->webSocket_->flush();
          lock->webSocket_ = nullptr;
          lock->canWriteWebSocket_ = false;
        }
                                           ...
                                           const std::string &se) const
  {
    ...
    if (!signalE)
    {
      ...
      for (Http::ParameterMap::const_iterator i = entries.begin();
           i != entries.end(); ++i)
      {
        if (i->first.length() > static_cast<unsigned>(signalLength) && i->first.substr(0, signalLength) == se + "signal=")
        {
          signalE = &i->second[0];
   ...
   */
     ...
     */
      if ((!requestE || (*requestE != "resource")) && handler.response()->responseType() == WebResponse::ResponseType::Page)
      {
        /*
        ...
        if (!env_->agentIsIE())
          if ((handler.request()->headerValue("user-agent")) != env_->userAgent())
          {
            ...
            LOG_INFO("new user agent: "
                     << std::string(handler.request()->headerValue("user-agent")));
            serveError(403, handler, "Forbidden");
            return;
          }
      ...
      else
      {
        ...
        if (!requestE)
        {
          if (!request.pathInfo().empty())
            resource = app_->decodeExposedResource("/path/" + Utils::prepend(request.pathInfo(), '/'));
          ...
          else
          {
            ...
            else
            {
              LOG_ERROR("decodeResource(): resource '" << *resourceE
                                                       << "' not exposed");
              handler.response()->setContentType("text/html");
              *handler.response() << "<html><body><h1>Nothing to say about that.</h1></body></html>";
              handler.flushResponse();
            }
          }
        }
              ...
              */
              if (!WebController::isAsyncSupported() && renderer_.jsSynced())
              {
                ...
                if (!updatesPending_)
                {
                  ...
                  try
                  {
                    updatesPendingEvent_.timed_wait(controller_->configuration().serverPushTimeout() * 2);
                  }
  ...
  void WebSession::changeInternalPath(std::string_view path,
                                      WebResponse *response)
  {
    ...
    case State::Loaded:
      ...
      else
      {
        ...
        if (!requestE && !request.pathInfo().empty())
          resource = app_->decodeExposedResource("/path/" + std::string(request.pathInfo()));
        ...
        else if (signalE)
        {
          ...
          {
            std::vector<unsigned int> signalOrder = getSignalProcessingOrder(event);
   ...
   */
    try
    {
      ...
      if (handler.response())
      { // a recursive eventloop may remove it in kill()
        updatesPending_ = false;
        serveResponse(handler);
      }
    }
   ...
   */
     ...
     */
      if (handler.response()->responseType() == WebResponse::ResponseType::Script && !handler.request()->getParameter("skeleton"))
      {
#ifndef WT_TARGET_JAVA
        if (bootStyleResponse_)
        {
  ...
  void WebSession::setPagePathInfo(std::string_view path)
  {
    if (!useUglyInternalPaths())
      pagePathInfo_ = path;
  }
