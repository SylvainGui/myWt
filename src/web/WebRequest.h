...
public:
   ...
   */
  virtual void append(const char *s, int length) { out().write(s, length); };
   ...
   */
  virtual void addHeader(const std::string& name, std::string_view value) = 0;
...
private:
  std::string parsePreferredAcceptValue(std::string_view value) const;
