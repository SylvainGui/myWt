...
std::string append(std::string_view s, char c)
{
  if (s.empty() || s[s.length() - 1] != c)
    return std::string(s.data()) + c;
  else
...
std::string urlEncode(std::string_view url, const std::string& allowed)
{
...
{
    long cv;
    auto result = std::from_chars(v.data(), v.data() + v.size(), cv);
    return cv;
}
...
double stod(const std::string_view v)
{
    ...
    auto value = boost::convert<double>(v, boost::cnv::strtol());
...
{
    //gcc 9-10 unreliable compliance to c++ 17 iso
    auto value = boost::convert<float>(v, boost::cnv::strtol());
    return value.value_or(0);
}
