...
namespace Wt
{
  ...
  {
    return !updateMap_.empty() || formObjectsChanged_ || session_.app()->hasQuit() || !session_.app()->afterLoadJavaScript_.empty() || session_.app()->serverPushChanged_ || session_.app()->styleSheetsAdded_ || !session_.app()->styleSheetsToRemove_.empty() || session_.app()->styleSheet().isDirty() || session_.app()->internalPathIsChanged_ || !collectedJS1_.empty() || !collectedJS2_.empty() || !invisibleJS_.empty() || !wsRequestsToHandle_.empty() || multiSessionCookieUpdateNeeded_;
  }
                                ...
                                bool embedded)
  {
    ...
    response << "if (window.Wt) window.Wt._p_.quit(null); window.location.reload(true);";
  }
                                     ...
                                     const std::string &redirect)
  {
    if (session_.app() && session_.app()->internalPathIsChanged_)
      out << "if (window." << session_.app()->javaScriptClass() << ") "
          ...
          << ", false);\n";
    out << "if (window.location.replace)"
           " window.location.replace("
        << WWebWidget::jsStringLiteral(redirect) << ");"
                                                    "else"
                                                    " window.location.href="
        << WWebWidget::jsStringLiteral(redirect) << ";\n";
  }

  void WebRenderer::streamRedirectJS(WebRequest& out, const std::string& redirect)
  {
    if (session_.app() && session_.app()->internalPathIsChanged_)
      out.out() << "if (window." << session_.app()->javaScriptClass() << ") "
          << session_.app()->javaScriptClass()
          << "._p_.setHash("
          << WWebWidget::jsStringLiteral(session_.app()->newInternalPath_)
          << ", false);\n";
    out.out() << "if (window.location.replace)"
           " window.location.replace("
        << WWebWidget::jsStringLiteral(redirect) << ");"
                                                    "else"
                                                    " window.location.href="
  ...
  void WebRenderer::setPageVars(FileServe &page)
  {
    ...
    page.setCondition("FORM", !session_.env().agentIsSpiderBot() && !session_.env().ajax());
                                      ...
                                      FileServe &boot, bool hybrid)
  {
    ...
    if (!(hybrid && session_.app()->hasQuit()))
    {
      ...
      bootJs.setVar("SELF_URL",
                    safeJsStringLiteral(session_.bootstrapUrl(response, WebSession::BootstrapOption::ClearInternalPath)));
      ...
      bootJs.setVar("PATH_INFO", safeJsStringLiteral(session_.pagePathInfo_));
      ...
      std::string internalPath = hybrid ? session_.app()->internalPath() : session_.env().internalPath();
  ...
  void WebRenderer::serveLinkedCss(WebResponse &response)
  {
    ...
    else if (linkedCssCount_ > -1)
    {
      ...
      unsigned count = std::min((std::size_t)linkedCssCount_, app->styleSheets_.size());
  ...
  void WebRenderer::serveBootstrap(WebResponse &response)
  {
    ...
    DomElement::htmlAttributeValue(noJsRedirectUrl,
                                   session_.bootstrapUrl(response,
    ...
    boot.setVar("AUTO_REDIRECT",
                "<noscript><meta http-equiv=\"refresh\" content=\"0; url=" + noJsRedirectUrl.str() + "\"></noscript>");
    boot.setVar("NOSCRIPT_TEXT", conf.redirectMessage());

    WStringStream bootStyleUrl;
    DomElement::htmlAttributeValue(bootStyleUrl,
                                   session_.bootstrapUrl(response,
                                                         WebSession::BootstrapOption::ClearInternalPath) +
  ...
  void WebRenderer::setHeaders(WebResponse &response, const std::string mimeType)
  {
    for (std::map<std::string, CookieValue>::const_iterator
             i = cookiesToSet_.begin();
         i != cookiesToSet_.end(); ++i)
    {
      ...
      if (!cookie.expires.isNull())
      {
        ...
        std::string d = cookie.expires.toString(WString::fromUTF8(formatString), false).toUTF8();
  ...
  {
    Configuration &conf = session_.controller()->configuration();
    setCookie("ms" + std::string(request.scriptName()),
   ...
   */
    ...
    if (serveSkeletons)
    {
      ...
      FileServe script(parts.size() > 1
                           ? Wt_js_combined.c_str()
                           : skeletons::Wt_js1);

      script.setCondition("CATCH_ERROR", conf.errorReporting() != Configuration::NoErrors);
      script.setCondition("SHOW_ERROR", conf.errorReporting() == Configuration::ErrorMessage);
      script.setCondition("UGLY_INTERNAL_PATHS", session_.useUglyInternalPaths());
      ...
      script.setVar("IDLE_TIMEOUT", conf.idleTimeout() != -1 ? std::to_string(conf.idleTimeout()) : std::string("null"));
 ...
 */
   ...
   */
    if (!app->environment().ajax() && (/*response.requestMethod() == "POST"
	  || */
                                       (app->internalPathIsChanged_ && app->renderedInternalPath_ != app->newInternalPath_)))
    {
   ...
   */
    ...
    std::string url = (app->environment().agentIsSpiderBot() || !session_.useUrlRewriting())
  ...
  std::string WebRenderer::headDeclarations() const
  {
    ...
    if (session_.app())
    {
      const std::vector<MetaHeader> &appMetaHeaders = session_.app()->metaHeaders_;
    ...
    {
      result << "<link rel=\"shortcut icon\" href=\"" << session_.favicon() << '"';
      closeSpecial(result);
    }
