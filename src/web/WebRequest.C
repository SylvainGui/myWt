...
{
  std::string_view lenstr = envValue("CONTENT_LENGTH");
  ...
  else
  {
    try
    {
      ...
      if (len < 0)
      {
        LOG_ERROR("Bad content-length: " << std::string(lenstr));
...
std::string WebRequest::parsePreferredAcceptValue(std::string_view str) const
{
  if (str.empty())
    return std::string();
  ...
  {
    LOG_ERROR("Could not parse 'Accept-Language: " << std::string(str)
                                                   << "', stopped at: '" << info.stop << '\'');
    return std::string();
  }
}
#else
std::string WebRequest::parsePreferredAcceptValue(std::string_view) const
{
  return std::string();
}
#endif // WT_NO_SPIRIT
...
{
  return WLocale("fr-FR");//parsePreferredAcceptValue(headerValue("accept-language"))
}
...
std::string WebRequest::clientAddress(const Configuration &conf) const
{
  ...
  else
  {
    ...
    {
      std::string forwardedFor(headerValue(conf.originalIPHeader().c_str()));
