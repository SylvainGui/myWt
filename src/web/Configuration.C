...
namespace
{
  ...
  xml_node<> *singleChildElement(xml_node<> *element, const char *tagName)
  {
    ...
    if (result)
    {
      ...
      if (next)
      {
        throw WServer::Exception(std::string("Expected only one child <") + tagName + "> in <" + element->name() + ">");
      }
    }
  ...
  std::string elementValue(xml_node<> *element, const char *elementName)
  {
    for (xml_node<> *e = element->first_node(); e; e = e->next_sibling())
      if (e->type() != node_data && e->type() != node_cdata)
        throw WServer::Exception(std::string("<") + elementName + "> should only contain text.");
  ...
  void setBoolean(xml_node<> *element, const char *tagName, bool &result)
  {
    ...
    if (!v.empty())
    {
      if (v == "true")
        ...
        throw WServer::Exception("<" + std::string(tagName) + ">: expecting 'true' or 'false'");
    }
  }
  ...
  void setInt(xml_node<> *element, const char *tagName, int &result)
  {
    ...
    if (!v.empty())
    {
      ...
      {
        throw WServer::Exception("<" + std::string(tagName) + ">: expecting integer value");
      }
    }
  }
...
namespace Wt
{
  ...
  void Configuration::registerEntryPoint(const EntryPoint &ep)
  {
    ...
    typedef boost::split_iterator<std::string_view::const_iterator> spliterator;
  ...
  void Configuration::removeEntryPoint(std::string_view path)
  {
  ...
  void Configuration::setDefaultEntryPoint(std::string_view path)
  {
    defaultEntryPoint_ = path;
  }

  EntryPointMatch Configuration::matchEntryPoint(std::string_view scriptName,
                                                 ...
                                                 bool matchAfterSlash) const
  {
    if (!scriptName.empty())
    {
      ...
      EntryPointMatch m = matchEntryPoint(EMPTY_STR, std::string(scriptName) + std::string(path), matchAfterSlash);
      if (m.entryPoint)
        return m;
      else
    ...
    if (entryPoints_.size() == 1 && entryPoints_[0].path().empty())
    {
  ...
  bool Configuration::matchesPath(std::string_view path,
  ...
  void Configuration::readApplicationSettings(xml_node<> *app)
  {
    ...
    std::string maxRequestStr = singleChildElementValue(app, "max-request-size", "");
   ...
   * would do such a thing ? */
    connectorSessionIdPrefix_ = singleChildElementValue(app, "session-id-prefix",
    ...
    std::string plainAjaxSessionsRatioLimit = singleChildElementValue(app, "plain-ajax-sessions-ratio-limit", "");
    ...
    for (unsigned i = 0; i < userAgents.size(); ++i)
    {
      ...
      if (type == "ajax")
      {
        ...
        if (mode == "black-list")
          ...
          throw WServer::Exception("<user-agents type=\"ajax\" requires attribute 'mode' with value "
                                   "\"white-list\" or \"black-list\"");
      }
      else if (type == "bot")
        list = &botList_;
      else
        throw WServer::Exception("<user-agents> requires attribute 'type' with value "
                                 "\"ajax\" or \"bot\"");

      std::vector<xml_node<> *> agents = childElements(userAgentsList, "user-agent");
    ...
    for (unsigned i = 0; i < metaHeaders.size(); ++i)
    {
      ...
      for (unsigned j = 0; j < metas.size(); ++j)
      {
        ...
        {
          throw WServer::Exception("<meta> requires attribute 'name', 'property' or 'http-equiv'");
        }
    ...
    std::string allowedOrigins = singleChildElementValue(app, "allowed-origins", "");
  ...
  void Configuration::readConfiguration(bool silent)
  {
    ...
    if (!s)
    {
      if (configurationFile_ != WT_CONFIG_XML)
        throw WServer::Exception("Error reading '" + configurationFile_ + "': could not open file.");
      else
        return;
    }
    ...
    {
      xml_document<> doc;
      doc.parse<parse_normalize_whitespace | parse_trim_whitespace | parse_validate_closing_tags>(text.get());
      ...
      std::vector<xml_node<> *> applications = childElements(root, "application-settings");
    ...
    {
      throw WServer::Exception("Error reading: " + configurationFile_ + ": " + e.what());
    }
    catch (...)
    {
