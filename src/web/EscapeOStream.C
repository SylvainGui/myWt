...
namespace Wt
{
  ...
  namespace Dbo
  {
    ...
    {
      if (rules.c_special_ == 0)
        stream_.append(s.data(), s.length());
      else
        put(s, rules);
    }
    ...
    void EscapeOStream::put(std::string_view sv, const EscapeOStream &rules)
    {
       std::string cd = std::string(sv);
      // std::cout << "string " << sv.length() << " - " << sv.size() << std::endl;
      ...
      // auto index = sv.find_first_of(rules.c_special_);
      // for (; s;)
      // {
      //   //const char *f = std::strpbrk(s, rules.c_special_);
      //   std::cout << " sdcsdc " << index << std::endl;
      //   if (index != std::string_view::npos)
      //   {
      //     auto f = sv.data() + index;
      //     std::cout << " fragment " << f - s << std::endl;
      //     stream_.append(s, static_cast<int>(f - s));
      ...
      //   else
      //   {
      //     auto f = sv.data() + sv.size();
      //     std::cout << " fragment " << f - s << std::endl;
      //     stream_ << std::string(sv);
      //     s = 0;
      //   }
      //   index = sv.find_first_of(rules.c_special_, index + 1);
      // }
    }
