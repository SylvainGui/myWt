...
std::cerr << "open " << s << " - " << httpContextData << " - " << 0 << std::endl;
...
std::cerr << "close " << s << " - " << httpResponseData << " - " << 0 << std::endl;
...
// , httpResponseData->lastWtReply_, 
//                                                                                                httpResponseData->lastProxyReply_, 
//                                                                                                httpResponseData->lastStaticReply_
...
public:
    ...
    void onHttp(std::string method, std::string pattern, MoveOnlyFunction<void(HttpResponse<SSL> *, HttpRequest *)> &&handler, bool upgrade = false) {
        ...
        httpContextData->router.add(methods, pattern, [handler = std::move(handler)](auto *r) mutable {
            ...
            // user.httpRequest->lastWtReply_ = user.lastWtReply_;
            // user.httpRequest->lastProxyReply_ = user.lastProxyReply_;
            // user.httpRequest->lastStaticReply_ = user.lastStaticReply_;
            // std::cerr << "test : " << user.lastWtReply_ << std::endl;
