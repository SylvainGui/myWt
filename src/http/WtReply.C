...
namespace http
{
  namespace server
  {
    ...
    void WtReply::reset(const Wt::EntryPoint *ep)
    {
      ...
      if (httpRequest_)
        httpRequest_->reset(std::static_pointer_cast<WtReply>(shared_from_this()), ep);
     ...
     */
       ...
       */
          if (!httpRequest_)
            httpRequest_ = new HTTPRequest(std::static_pointer_cast<WtReply>(shared_from_this()), entryPoint_);

          if (end - begin > 0)
          {
            bodyReceived_ += (end - begin);

            if (!connection()->server()->controller()->requestDataReceived(httpRequest_, bodyReceived_, request().contentLength))
            {
              delete httpRequest_;
              httpRequest_ = nullptr;
        ...
        if (state != Request::Partial)
        {
          ...
          else
          {
            ...
            if (entryPoint_->resource())
              ...
              connection()->server()->service().post(std::bind(&Wt::WebController::handleRequest,
                                          ...
                                          Request::State state)
    {
          ...
          configuration().maxMemoryRequestSize())
      {
        LOG_ERROR("Rejecting WebSocket message because it exceeds "
                  "--max-memory-request-size (= "
                  << configuration().maxMemoryRequestSize() << " bytes)");
        state = Request::Error;
      }
      else
      {
    ...
    void WtReply::formatResponse(std::vector<asio::const_buffer> &result)
    {
      ...
      if (webSocket)
      {
        ...
        case 13:
        {
          ...
          else
          {
            ...
            for (unsigned i = 0; i < SizeTLength; ++i)
              gatherBuf_[j++] = (char)(payloadLength >> ((SizeTLength - 1 - i) * 8));
        ...
        break;
      default:
        LOG_ERROR("ws: encoding for version " << request().webSocketVersion << " is not implemented");
    ...
    else result.push_back(out_buf_.data());
  }
  ...
  bool WtReply::initDeflate()
  {
    ...
    int wsize = request_.pmdState_.server_max_window_bits != -1 ? request_.pmdState_.server_max_window_bits : SERVER_MAX_WINDOW_BITS;
