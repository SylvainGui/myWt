    ...
    void StaticReply::reset(const Wt::EntryPoint *ep)
    {
      ...
      if (hasRange_)
      {
        ...
        if (curpos != rangeBegin_)
        {
          ...
          ReplyPtr sr(new StockReply(request_, StockReply::requested_range_not_satisfiable,
                                     "", configuration()));
          if (fileSize_ != -1)
          {
   ...
   */
    ...
    bool StaticReply::nextContentBuffers(std::vector<asio::const_buffer> &result)
    {
      if (request_.method != "HEAD")
      {
        ...
        stream_.read(buf_, (std::streamsize)(std::min<boost::uintmax_t>)(rangeRemainder, sizeof(buf_)));
