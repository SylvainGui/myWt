...
namespace http
{
  namespace server
  {
    ...
    void Connection::handleReadRequest0()
    {
      ...
      try
      {
        LOG_DEBUG(socket().native_handle() << "incoming request: "
                                           ...
                                           << std::string(rcv_remaining_,
                                                          std::min((unsigned long)(buffer.data() - rcv_remaining_ + rcv_buffer_size_),
                                                                   (long unsigned)1000)));
      }
      catch (...)
      {
      ...
      boost::tie(result, rcv_remaining_) = request_parser_.parse(request_,
    ...
    {
      // ReplyPtr reply(new StockReply(request_, status, "", server_->configuration()));
    ...
    bool Connection::readAvailable()
    {
      try
      {
        return (rcv_remaining_ < rcv_buffers_.back().data() + rcv_buffer_size_) || socket().available();
      }
      catch (Wt::AsioWrapper::system_error &e)
      {
   ...
   */
                                     ...
                                     std::size_t bytes_transferred)
    {
      ...
      else if (e != asio::error::operation_aborted && e != asio::error::bad_descriptor)
      {
    ...
    {
      LOG_DEBUG(native() << ": handleWriteResponse() " << haveResponse_ << " " << responseDone_);
      if (haveResponse_)
        startWriteResponse(reply);
      else
      {
